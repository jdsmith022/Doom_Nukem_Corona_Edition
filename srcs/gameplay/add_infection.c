...
int		sub_section(t_doom *doom, int i, double *x_steps, double *y_steps)
{
	...
	x_diff = (doom->lib.sidedef[i].line.end.x) -
	(doom->lib.sidedef[i].line.start.x);
	y_diff = (doom->lib.sidedef[i].line.end.y) -
	(doom->lib.sidedef[i].line.start.y);
...
void	init_infection(t_sidedef *infection, t_doom *doom)
{
	...
	infection->line.start.x = change * x_steps +
	doom->lib.sidedef[i].line.start.x;
	infection->line.start.y = change * y_steps +
	doom->lib.sidedef[i].line.start.y;
...
void	add_infection(t_doom *doom)
{
	if (rand() % 10 == 1 && doom->lib.cur_len_infection < 100)
	{
		...
		init_infection(
			&(doom->lib.infection[doom->lib.cur_len_infection]), doom);
	}
}
