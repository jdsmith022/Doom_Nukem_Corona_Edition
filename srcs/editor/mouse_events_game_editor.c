...
void    mouse_press_map(t_doom *doom, int x, int y)
{
	...
	// 	doom->game_design.sector[doom->game_design.cur_sec].diff_y -= 10;
  	if (doom->game_design.pl_pos == 0 && x > SIDEBAR_SECTOR)
...
void	sidedef_check(t_doom *doom, int x, int y)
{
	if (x > PORTAL_X && x < PORTAL_X + FRAME_WIDTH && y > PORTAL_Y && y < PORTAL_Y + FRAME_HEIGHT)
	{
		if (doom->game_design.s_len == 0)
			add_sector(doom);
		add_portal(doom, 0);
...
void	sector_check(t_doom *doom, int x, int y)
{
	if (x > AR_RIGHT_TF_X && x < AR_RIGHT_TF_X + FRAME_WIDTH && y > AR_RIGHT_TF_Y && y < AR_RIGHT_TF_Y + FRAME_HEIGHT)
	{
		if (doom->lib.len_tex_lib  > doom->game_design.sector[doom->game_design.cur_sec].txt_floor + 1)
		doom->game_design.sector[doom->game_design.cur_sec].txt_floor++;
	}
...
void	button_check(t_doom *doom, int x, int y)
{
	if (x > SECTOR_BOTTON_START_X && x < SECTOR_BOTTON_END_X && \
		y > SECTOR_BOTTON_START_Y && x < SECTOR_BOTTON_END_Y)
	{
		doom->game_design.b_sector = doom->game_design.b_sector == TRUE ? FALSE : TRUE;
		doom->game_design.b_sidedef = FALSE;
		doom->game_design.b_object = FALSE;
	}
	if (x > SIDEDEF_BOTTON_START_X && x < SIDEDEF_BOTTON_END_X && \
		y > SIDEDEF_BOTTON_START_Y && x < SIDEDEF_BOTTON_END_Y)
	{
		doom->game_design.b_sidedef = doom->game_design.b_sidedef == TRUE ? FALSE : TRUE;
		doom->game_design.b_sector = FALSE;
		doom->game_design.b_object = FALSE;
	}	
	if (x > OBJECT_BOTTON_START_X && x < OBJECT_BOTTON_END_X && \
		y > OBJECT_BOTTON_START_Y && x < OBJECT_BOTTON_END_Y)
	{
		doom->game_design.b_object = doom->game_design.b_sector == TRUE ? FALSE : TRUE;
		doom->game_design.b_sidedef = FALSE;
		doom->game_design.b_sector = FALSE;
	}
}
