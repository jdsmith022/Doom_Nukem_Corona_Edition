...
t_angle_line calc_angle(t_line line, t_doom *doom)
{
	...
	angle.start.x = line.start.x - doom->game_design.sector[doom->game_design.cur_sec].diff_x;
	angle.start.y = line.start.y - doom->game_design.sector[doom->game_design.cur_sec].diff_y;
	return (angle);
}
...
void	draw_lines(t_doom *doom, Uint32 **pixels, int b)
{
	...
	if (doom->game_design.sidedef[b].opp_sidedef != -1 && doom->game_design.cur_sd != b)
		color = 0xffFFA07A;
	else if (doom->game_design.cur_sd != b || doom->game_design.sidedef_bar == 0)
		color = 0x8c3cde6;
	else
		color = 0xff4287f5;
	while ((int)i < abs(angle.diff.x) + abs(angle.diff.y))
