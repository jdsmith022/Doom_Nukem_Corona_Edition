...
void    add_to_game(t_doom *doom)
{
	...
	// 	printf("%f %f %f %f\n", doom->game_design.sidedef[x].line.start.x,  doom->game_design.sidedef[x].line.start.y,  doom->game_design.sidedef[x].line.end.x,  doom->game_design.sidedef[x].line.end.y);
    	for (int x = 0; x < doom->game_design.w_len; x++)
