...
Uint32	init_put_texture(t_doom *doom, int index, t_point *change, SDL_Surface **texture)
{
	Uint32 shift;

	*texture = doom->game_design.sidedef_bar == 1 ? \
	doom->lib.tex_lib[index] : doom->lib.obj_lib[index];
	shift = texture[0]->format->BitsPerPixel == 24 ? 3 : 4;
	change[0].x = (double)texture[0]->w / 48.0;
	change[0].y = (double)texture[0]->h / 48.0;
	return (shift);
}
void	put_textures(int x, int y, int index, t_doom *doom)
{
	...
	save_x = x;
	shift = init_put_texture(doom, index, &change, &texture);
	i.x = 0;
	i.y = 0;
...
void    put_images(int x, int y, int index, t_doom *doom)
{
	...
	i.x = 0;
	i.y = 0;
	shift = doom->game_design.sym_lib[index]->format->BitsPerPixel == 24\
	? 3 : 4;
	...
	{
		put_symbol(doom, (Uint32)index, (Uint32)((y * WIDTH + x) * 4), (Uint32)\
		(((int)i.y * doom->game_design.sym_lib[index]->w + (int)i.x) * shift));
		x++;
		i.x += change.x;
