...
void	open_game_editor(t_doom *doom)
{
	...
	bars(&pixels, doom);
	i = doom->lib.sector[doom->game_design.cur_sec].i_sidedefs;
	while (i < doom->lib.sector[doom->game_design.cur_sec].i_sidedefs\
		+ doom->lib.sector[doom->game_design.cur_sec].n_sidedefs)
	{
