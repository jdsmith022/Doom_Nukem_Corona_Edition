...
void	mouse_press_sidedef_txt(t_doom *doom, int x, int y)
{
	...
	x > AR_LEFT_TS1_X && x < AR_LEFT_TS1_X + FRAME_WIDTH && \
	y > AR_LEFT_TS1_Y && y < AR_LEFT_TS1_Y + FRAME_HEIGHT)
		change_texture(doom, &(doom->game_design.sidedef[doom->game_design.cur_sd].txt_1), -1);
	else if (doom->game_design.sidedef[doom->game_design.cur_sd].opp_sidedef != -1 && \
	x > AR_RIGHT_TS1_X && x < AR_RIGHT_TS1_X + FRAME_WIDTH && \
	...
	y > AR_LEFT_TS2_Y && y < AR_LEFT_TS2_Y + FRAME_HEIGHT)
		change_texture(doom, &(doom->game_design.sidedef[doom->game_design.cur_sd].txt_2), -1);
	else if (x > AR_RIGHT_TS2_X && x < AR_RIGHT_TS2_X + FRAME_WIDTH && \
	y > AR_RIGHT_TS2_Y && y < AR_RIGHT_TS2_Y + FRAME_HEIGHT)
		change_texture(doom, &(doom->game_design.sidedef[doom->game_design.cur_sd].txt_2), 1);
	else if (doom->game_design.sidedef[doom->game_design.cur_sd].opp_sidedef != -1 \
	&& x > AR_LEFT_TS3_X && x < AR_LEFT_TS3_X + FRAME_WIDTH && \
	y > AR_LEFT_TS3_Y && y < AR_LEFT_TS3_Y + FRAME_HEIGHT)
		change_texture(doom, &(doom->game_design.sidedef[doom->game_design.cur_sd].txt_3), -1);
	else if (doom->game_design.sidedef[doom->game_design.cur_sd].opp_sidedef != -1 && \
	x > AR_RIGHT_TS3_X && x < AR_RIGHT_TS3_X + FRAME_WIDTH && \
