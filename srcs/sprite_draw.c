...
/*   Updated: 2020/07/07 17:24:48 by rooscocolie   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_stripes(t_doom *doom, t_point sprite_cord, int index_sp)
{
	...
	i_sprite = doom->sprite[index_sp].index;
	begin.x = sprite_cord.x - doom->sprite[index_sp].width / 2;
	begin.y = sprite_cord.y - doom->sprite[index_sp].height / 2;
	end.x = begin.x + doom->sprite[index_sp].width;
	end.y = begin.y + doom->sprite[index_sp].height;
	stripe = begin.x;
	pix_y = 0;
	pix_x = 0;
	screen_y = begin.y;
	...
	while (stripe < end.x && begin.x > 0 && stripe < WIDTH)
	{
		...
		while (pix_y < (int)end.y/* && pix_y > 0 && pix_y < HEIGHT*/)
		{
			...
			pix_dex = (pix_y * doom->sprite_image[i_sprite]->pitch) + (pix_x * doom->sprite_image[i_sprite]->format->BytesPerPixel);
