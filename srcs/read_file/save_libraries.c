...
void		reset_sidedef_id(t_lib *lib)
{
	int i;

	i = 0;
	while (i < lib->len_sidedef)
	{
		lib->sidedef[i].id = i;
		i++;
	}
...
static void		modified(void)
{
	...
	stat("srcs/read_file/start_skybox-1", &filestat);
...
void		save_libraries(t_doom *doom)
{
	...
	modified();
	fd = open_file("srcs/read_file/start_skybox-1");
