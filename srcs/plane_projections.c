...
/*   Updated: 2020/05/28 16:56:03 by Malou         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
	...
	int opp_sector, t_plane *plane)
{
	...
	height_floor = doom->sector[opp_sector].height_floor /\
		sidedef.distance * doom->dist_to_plane;
	mid_top = (int)((HEIGHT / 2) - (height_standard / 2) -\
		(height_opp_sector - height_standard));
	plane->mid_texture_top = ((mid_top >= 0) ? mid_top : 0);
	mid_bottom = (int)((HEIGHT / 2) + (height_standard / 2) - height_floor);
	...
	t_sector sector, t_plane *plane)
{
	...
	height_floor = sector.height_floor /\
		sidedef.distance * doom->dist_to_plane;
	sidedef_top = (int)((HEIGHT / 2) - (height_standard / 2)\
		- (height_sidedef - height_standard));
	plane->sidedef_top = ((sidedef_top >= 0) ? sidedef_top : 0);
	sidedef_bottom = (int)((HEIGHT / 2) + (height_standard / 2) - height_floor);
