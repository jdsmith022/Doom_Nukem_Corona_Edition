...
/*   Updated: 2020/05/18 18:30:42 by jessicasmit   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
					...
					t_sector sector, t_plane *plane)
{
	...
	sidedef_top = (int)(HEIGHT / 2 - plane->sidedef_height / 2);
					...
					t_sidedef sidedef, t_sector sector)
{
	...
	ceiling_height = doom->wall_height / sidedef.distance * doom->dist_to_plane;
	diff = doom->wall_height - sector.height_ceiling; 
	ceiling_diff = diff / sidedef.distance * doom->dist_to_plane;
	ceiling_start = (int)(HEIGHT / 2 - (ceiling_height - ceiling_diff) / 2);
					...
					t_sidedef sidedef, t_sector sector)
{
	...
	wall_height = doom->wall_height / sidedef.distance * doom->dist_to_plane;
	floor_diff =  sector.height_floor / sidedef.distance * doom->dist_to_plane;
	floor_start = (int)(HEIGHT / 2 + (wall_height - floor_diff) / 2);
