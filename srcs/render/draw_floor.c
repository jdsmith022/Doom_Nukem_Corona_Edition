...
double		calculate_new_floor_height(t_doom *doom, t_sector *sector)
{
	...
	new_intersect.x = sector->slope.prev_intersect.x + (sector->slope.dist_to_bottom * cos(doom->ray_angle));
	new_intersect.y = sector->slope.prev_intersect.y + (sector->slope.dist_to_bottom * sin(doom->ray_angle));
