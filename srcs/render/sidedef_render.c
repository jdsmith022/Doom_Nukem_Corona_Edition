						...
						t_sidedef curr_sidedef, t_doom *doom)
{
	...
	doom->cast.texture_width = doom->lib.tex_lib[sidedef.txt_2]->w;
	doom->cast.texture_height = doom->lib.tex_lib[sidedef.txt_2]->h;
					...
					t_render render, int sector)
{
	...
	doom->cast.distance = render.min_distance;
					...
					int prev_sector)
{
		...
		doom->lib.sector[sector].i_sidedefs)
	{
		...
		if (render.distance <= (render.min_distance + 0.01) &&\
